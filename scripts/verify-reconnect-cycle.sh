...
PSVR2_STARTUP_RETRY_ATTEMPTS=1 \
PSVR2_STARTUP_RETRY_BASE_SECONDS=1 \
PSVR2_STARTUP_RETRY_MAX_SECONDS=2 \
...
if (( start_count != cycles || stop_count != cycles )); then
    echo "Reconnect cycle count mismatch: expected $cycles starts and stops, got $start_count/$stop_count" >&2
    echo "--- monitor log ---" >&2
    cat "$monitor_log" >&2
    exit 1
fi

echo "PSVR2 reconnect cycle test passed: $cycles connect/disconnect cycles with deterministic startup transitions."
